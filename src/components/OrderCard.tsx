...
const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
  ...
  const getStatusBadge = () => {
    switch (order.status) {
      case 'pending':
        return <Badge variant="outline" className="bg-yellow-100 text-yellow-800 text-base px-3 py-1">Pending</Badge>;
      case 'assigned':
        return <Badge variant="outline" className="bg-blue-100 text-blue-800 text-base px-3 py-1">Assigned</Badge>;
      case 'completed':
        return <Badge variant="outline" className="bg-green-100 text-green-800 text-base px-3 py-1">Completed</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <Card className={`order-card ${
      ...
      <CardContent className="pt-4">
        <div className={`flex ${isMobile ? 'flex-col gap-2' : 'justify-between items-start'} mb-4`}>
        ...
        <div className="space-y-4 text-base md:text-lg">
          ...
          <div className="flex items-center gap-3">
            <Phone size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
            <span className="font-medium">{order.contactNumber}</span>
          </div>
          
          <div className="mt-4">
            <div className="flex items-center gap-3 mb-3">
              <Table size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
            ...
            <div className="space-y-3">
              {order.tables && order.tables.length > 0 ? (
                order.tables.map((table, index) => (
                  <div key={table.id || index} className="border p-3 md:p-4 rounded-lg bg-gray-50">
                    <div className={`${isMobile ? 'grid grid-cols-1 gap-3' : 'grid grid-cols-2 gap-y-2'} text-base md:text-lg`}>
                      <div className="flex items-center gap-2">
                        <Package size={isMobile ? 18 : 20} className="flex-shrink-0 text-muted-foreground" />
                        <span className="font-medium">Size: {getTableSizeLabel(table.size)}</span>
                      </div>
                      
                      <div className="flex items-center gap-2">
                        <Palette size={isMobile ? 18 : 20} className="flex-shrink-0 text-muted-foreground" />
                        <span className="font-medium">Colour: {getColourLabel(table.colour)}</span>
                      </div>
                      
                      <div className="flex items-center gap-2">
                        <Hash size={isMobile ? 18 : 20} className="flex-shrink-0 text-muted-foreground" />
                        <span className="font-medium">Quantity: {table.quantity} {table.quantity > 1 ? 'tables' : 'table'}</span>
                      </div>
                      
                      <div className={`${isMobile ? 'text-left' : 'text-right'} font-bold`}>
          ...
          {order.note && (
            <div className="flex items-center gap-3 mt-3">
              <StickyNote size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
              <span className="font-medium break-words">{order.note}</span>
            </div>
          )}
          
          <div className="flex items-center gap-3 mt-3">
            <Calendar size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
            <span className="font-medium">Created: {format(new Date(order.createdAt), 'MMM d, yyyy')}</span>
          </div>
          
          {order.status === 'assigned' && (
            <div className="flex items-center gap-3 mt-3">
              <Truck size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
          ...
          {order.status === 'completed' && order.completedAt && (
            <div className="flex items-center gap-3 mt-3">
              <CheckCircle2 size={isMobile ? 20 : 24} className="flex-shrink-0 text-muted-foreground" />
